...
public class GallopMergeSort extends Sort {
    ...
    private static <T> void leftGalloping(T[] array, SortState<T> state,
        ...
        if (state.lessThanEqual.lessThan(lval, rval)) {
            int end = Util.binSearch(m.left, m.lcur + 1, m.llast, rval, 
                                     state.lessThan);
            ...
            int end = Util.binSearch(m.right, m.rcur + 1, m.rlast, lval, 
                                     state.lessThanEqual);
    ...
    private static <T> void rightGalloping(T[] array, SortState<T> state,
        ...
        T rval = m.right[m.rcur - 1];
        if (state.lessThan.lessThan(rval, lval)) {
            int begin = Util.binSearch(m.left, m.lfirst, m.lcur - 1, rval,
                                       state.lessThan);
            ...
            int begin = Util.binSearch(m.right, m.rfirst, m.rcur - 1, lval,
                                       state.lessThanEqual);
